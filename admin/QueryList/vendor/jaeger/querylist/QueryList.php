...
class QueryList
{
     ...
     * @param string $outputEncoding【输出编码格式】指要以什么编码输出(UTF-8,utf-8,.....)，防止出现乱码,如果设置为 假值 则不改变原字符串编码
     * @param string $inputEncoding 【输入编码格式】明确指定输入的页面编码格式(UTF-8,utf-8,.....)，防止出现乱码,如果设置为 假值 则自动识别
    ...
    {
        return mb_detect_encoding($string, array('ASCII', 'utf-8', 'GBK', 'UTF-8'));
    }
